"""
Automation Manager for Arasan Gas Middleware
Manages automated fetch and sync operations with configurable intervals.

Two-thread model (bol_company pattern):
  Thread 1: Daily master sync — fetch + sync customers & products once per day
  Thread 2: Invoice fetch + sync — combined fetch/sync every N seconds
"""
...
class AutomationManager:
        ...
        with self.lock:
            ...
            if self.state['status'] == 'running':
                ...
                for task in ['fetch_master', 'fetch_invoices', 'sync', 'sync_master']:
                    if self.state['next_runs'].get(task):
    ...
    def start(self):
        ...
        # Thread 2: Invoice fetch + sync — combined loop every N seconds
    ...
    # ------------------------------------------------------------------
    # Thread 1: Daily master sync (bol_company pattern)
    # ------------------------------------------------------------------
    ...
    def _daily_master_sync_loop(self):
        ...
        Runs immediately on first startup, then daily. Retries on failure.
        """
        ...
        # Parse daily sync time from config (default 02:00)
    ...
    # ------------------------------------------------------------------
    # Thread 2: Combined invoice fetch + sync loop (bol_company pattern)
    # ------------------------------------------------------------------
    ...
    def _invoice_fetch_sync_loop(self):
        ...
        4. Sync unsynced invoices/DCs to Catalytics
