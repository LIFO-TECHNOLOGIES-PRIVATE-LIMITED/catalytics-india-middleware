...
This creates `bol.sqlite` with all required tables.
...
*/10 * * * * cd /path/to/bol && python fetch_customers.py
*/10 * * * * cd /path/to/bol && python fetch_products.py

# Fetch invoices every 30 seconds
* * * * * cd /path/to/bol && python fetch_invoices.py
* * * * * sleep 30 && cd /path/to/bol && python fetch_invoices.py

# Sync to Catalytics every 30 seconds
* * * * * cd /path/to/bol && python sync_to_catalytics.py
* * * * * sleep 30 && cd /path/to/bol && python sync_to_catalytics.py
```
...
All operations are logged to `logs/bol.log`:
...
# View last 100 lines
tail -100 logs/bol.log

# Search for errors
grep "ERROR" logs/bol.log

# Search for duplicates
grep "DUPLICATE SKIPPED" logs/bol.log
```
...
├── bol.sqlite            # SQLite database (created on first run)
├── logs/
│   └── bol.log           # Application logs
