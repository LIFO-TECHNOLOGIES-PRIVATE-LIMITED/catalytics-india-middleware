...
TALLY_COMPANY_ACTIVE=bol
TALLY_COMPANY_bol=BOL Company Name
...
- Auto-create `bol.sqlite` database if missing
...
2. **bol.sqlite** database with tables:
...
```batch
# Build new release
cd C:\Github\catalytics-india-middleware\bol
build_release.bat
```

### Output Location
```
C:\Github\catalytics-india-middleware\bol\release\
└── bol_client_release_YYYYMMDD_HHMM\
