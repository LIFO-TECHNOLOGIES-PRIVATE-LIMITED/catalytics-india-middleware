...
from tally_middleware.fetch_invoices import build_config as build_fetch_config, run_once as fetch_once
...
logger = logging.getLogger("tally_invoice_loop")


def main() -> int:
    parser = argparse.ArgumentParser(description="Run Tally sales-invoice fetch and Catalytics DC sync in a loop.")
    ...
    while True:
        if args.mode in ("fetch", "both"):
            ...
            logger.info("Starting invoice fetch run")
            fetch_stats = fetch_once(fetch_config)
            logger.info("Invoice fetch complete: %s", fetch_stats)
