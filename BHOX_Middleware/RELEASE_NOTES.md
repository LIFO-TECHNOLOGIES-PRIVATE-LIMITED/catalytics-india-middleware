...
```
bol_client_release_YYYYMMDD_HHMM/
├── bol_dashboard.exe
├── .env
├── .env.example
├── logs/ (empty folder)
...
TALLY_COMPANY_ACTIVE=bhox
TALLY_COMPANY_bol=BHOX Company Name
...
### 3. Launch Dashboard
**Option 1**: Double-click `bol_dashboard.exe`
**Option 2**: Run `Start_Dashboard.bat`

The dashboard will:
- Auto-create `logs/` folder if missing
- Auto-create `bhox.sqlite` database if missing
...
2. **bhox.sqlite** database with tables:
...
```batch
# Build new release
cd C:\Github\catalytics-india-middleware\bhox
build_release.bat
```

### Output Location
```
C:\Github\catalytics-india-middleware\bhox\release\
└── bol_client_release_YYYYMMDD_HHMM\
    └── bol_client_release_YYYYMMDD_HHMM.zip
```

### What's Included
- `bol_dashboard.exe` - Main executable (no console window)
