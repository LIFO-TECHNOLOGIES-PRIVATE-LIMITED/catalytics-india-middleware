...
This creates `bhox.sqlite` with all required tables.
...
*/10 * * * * cd /path/to/bhox && python fetch_customers.py
*/10 * * * * cd /path/to/bhox && python fetch_products.py

# Fetch invoices every 30 seconds
* * * * * cd /path/to/bhox && python fetch_invoices.py
* * * * * sleep 30 && cd /path/to/bhox && python fetch_invoices.py

# Sync to Catalytics every 30 seconds
* * * * * cd /path/to/bhox && python sync_to_catalytics.py
* * * * * sleep 30 && cd /path/to/bhox && python sync_to_catalytics.py
```
...
All operations are logged to `logs/bhox.log`:

```bash
# View live logs
tail -f logs/bhox.log

# View last 100 lines
tail -100 logs/bhox.log

# Search for errors
grep "ERROR" logs/bhox.log

# Search for duplicates
grep "DUPLICATE SKIPPED" logs/bhox.log
```
...
├── bhox.sqlite            # SQLite database (created on first run)
├── logs/
│   └── bhox.log           # Application logs
