"""
SQLite Database Schema for BHOX Middleware
...
class Database:
    ...
    def __init__(self, db_path='bhox.sqlite'):
    ...
    def mark_invoices_deleted(self, company_name, voucher_nos):
        ...
        try:
            ...
            for i in range(0, len(voucher_list), 500):
                ...
                cursor = self.conn.cursor()
                cursor.execute(f"UPDATE invoices SET is_deleted = 1, deleted_at = CURRENT_TIMESTAMP WHERE tally_company = ? AND tally_voucher_no IN ({placeholders}) AND COALESCE(is_deleted, 0) = 0", [company_name] + batch)
    ...
    def log_duplicate(self, entity_type, entity_name, tally_company, owned_by_company, details=''):
        ...
        existing = self.query(
            "SELECT 1 FROM duplicate_log WHERE entity_type = ? AND entity_name = ? AND tally_company = ? AND owned_by_company = ? LIMIT 1",
